...
pub const MCO_DEAD: c_int = 0;
pub const MCO_NORMAL: c_int = 1;
pub const MCO_RUNNING: c_int = 2;
pub const MCO_SUSPENDED: c_int = 3;
pub const enum_mco_state = c_uint;
pub const mco_state = enum_mco_state;
pub const MCO_SUCCESS: c_int = 0;
pub const MCO_GENERIC_ERROR: c_int = 1;
pub const MCO_INVALID_POINTER: c_int = 2;
pub const MCO_INVALID_COROUTINE: c_int = 3;
pub const MCO_NOT_SUSPENDED: c_int = 4;
pub const MCO_NOT_RUNNING: c_int = 5;
pub const MCO_MAKE_CONTEXT_ERROR: c_int = 6;
pub const MCO_SWITCH_CONTEXT_ERROR: c_int = 7;
pub const MCO_NOT_ENOUGH_SPACE: c_int = 8;
pub const MCO_OUT_OF_MEMORY: c_int = 9;
pub const MCO_INVALID_ARGUMENTS: c_int = 10;
pub const MCO_INVALID_OPERATION: c_int = 11;
pub const MCO_STACK_OVERFLOW: c_int = 12;
pub const enum_mco_result = c_uint;
pub const mco_result = enum_mco_result;
pub const mco_coro = struct_mco_coro;
pub const struct_mco_coro = extern struct {
    context: ?*anyopaque,
    state: mco_state,
    func: ?*const fn ([*c]mco_coro) callconv(.C) void,
    prev_co: [*c]mco_coro,
    user_data: ?*anyopaque,
    allocator_data: ?*anyopaque,
    free_cb: ?*const fn (?*anyopaque, ?*anyopaque) callconv(.C) void,
    ...
    tsan_prev_fiber: ?*anyopaque,
    tsan_fiber: ?*anyopaque,
    magic_number: usize,
};
pub const struct_mco_desc = extern struct {
    func: ?*const fn ([*c]mco_coro) callconv(.C) void,
    user_data: ?*anyopaque,
    malloc_cb: ?*const fn (usize, ?*anyopaque) callconv(.C) ?*anyopaque,
    free_cb: ?*const fn (?*anyopaque, ?*anyopaque) callconv(.C) void,
    allocator_data: ?*anyopaque,
    storage_size: usize,
    coro_size: usize,
...
pub const mco_desc = struct_mco_desc;
pub export fn mco_desc_init(arg_func: ?*const fn ([*c]mco_coro) callconv(.C) void, arg_stack_size: usize) mco_desc {
    var func = arg_func;
    var stack_size = arg_stack_size;
    if (stack_size != @bitCast(c_ulong, @as(c_long, @as(c_int, 0)))) {
        if (stack_size < @bitCast(c_ulong, @as(c_long, @as(c_int, 32768)))) {
            stack_size = @bitCast(usize, @as(c_long, @as(c_int, 32768)));
        }
    } else {
        stack_size = @bitCast(usize, @as(c_long, @as(c_int, 57344)));
    }
    stack_size = _mco_align_forward(stack_size, @bitCast(usize, @as(c_long, @as(c_int, 16))));
    var desc: mco_desc = undefined;
    _ = memset(@ptrCast(?*anyopaque, &desc), @as(c_int, 0), @sizeOf(mco_desc));
    desc.malloc_cb = &mco_malloc;
    desc.free_cb = &mco_free;
    desc.func = func;
    desc.storage_size = @bitCast(usize, @as(c_long, @as(c_int, 1024)));
    _mco_init_desc_sizes(&desc, stack_size);
    return desc;
}
pub export fn mco_init(arg_co: [*c]mco_coro, arg_desc: [*c]mco_desc) mco_result {
    var co = arg_co;
    var desc = arg_desc;
    if (!(co != null)) {
        _ = puts("attempt to initialize an invalid coroutine");
        return @bitCast(c_uint, MCO_INVALID_COROUTINE);
    }
    _ = memset(@ptrCast(?*anyopaque, co), @as(c_int, 0), @sizeOf(mco_coro));
    var res: mco_result = _mco_validate_desc(desc);
    if (res != @bitCast(c_uint, MCO_SUCCESS)) return res;
    res = _mco_create_context(co, desc);
    if (res != @bitCast(c_uint, MCO_SUCCESS)) return res;
    co.*.state = @bitCast(c_uint, MCO_SUSPENDED);
    co.*.free_cb = desc.*.free_cb;
    co.*.allocator_data = desc.*.allocator_data;
    co.*.func = desc.*.func;
    co.*.user_data = desc.*.user_data;
    co.*.magic_number = @bitCast(usize, @as(c_long, @as(c_int, 2117906857)));
    return @bitCast(c_uint, MCO_SUCCESS);
}
pub export fn mco_uninit(arg_co: [*c]mco_coro) mco_result {
    var co = arg_co;
    if (!(co != null)) {
        _ = puts("attempt to uninitialize an invalid coroutine");
        return @bitCast(c_uint, MCO_INVALID_COROUTINE);
    }
    if (!((co.*.state == @bitCast(c_uint, MCO_SUSPENDED)) or (co.*.state == @bitCast(c_uint, MCO_DEAD)))) {
        _ = puts("attempt to uninitialize a coroutine that is not dead or suspended");
        return @bitCast(c_uint, MCO_INVALID_OPERATION);
    }
    co.*.state = @bitCast(c_uint, MCO_DEAD);
    _mco_destroy_context(co);
    return @bitCast(c_uint, MCO_SUCCESS);
}
pub export fn mco_create(arg_out_co: [*c][*c]mco_coro, arg_desc: [*c]mco_desc) mco_result {
    var out_co = arg_out_co;
    var desc = arg_desc;
    if (!(out_co != null)) {
        _ = puts("coroutine output pointer is NULL");
        return @bitCast(c_uint, MCO_INVALID_POINTER);
    }
    if ((!(desc != null) or !(desc.*.malloc_cb != null)) or !(desc.*.free_cb != null)) {
        out_co.* = null;
        _ = puts("coroutine allocator description is not set");
        return @bitCast(c_uint, MCO_INVALID_ARGUMENTS);
    }
    var co: [*c]mco_coro = @ptrCast([*c]mco_coro, @alignCast(@import("std").meta.alignment([*c]mco_coro), desc.*.malloc_cb.?(desc.*.coro_size, desc.*.allocator_data)));
    if (!(co != null)) {
        _ = puts("coroutine allocation failed");
        out_co.* = null;
        return @bitCast(c_uint, MCO_OUT_OF_MEMORY);
    }
    var res: mco_result = mco_init(co, desc);
    if (res != @bitCast(c_uint, MCO_SUCCESS)) {
        desc.*.free_cb.?(@ptrCast(?*anyopaque, co), desc.*.allocator_data);
        out_co.* = null;
        return res;
    }
    out_co.* = co;
...
pub export fn mco_destroy(arg_co: [*c]mco_coro) mco_result {
    var co = arg_co;
    if (!(co != null)) {
        _ = puts("attempt to destroy an invalid coroutine");
        return @bitCast(c_uint, MCO_INVALID_COROUTINE);
    }
    var res: mco_result = mco_uninit(co);
    if (res != @bitCast(c_uint, MCO_SUCCESS)) return res;
    if (!(co.*.free_cb != null)) {
        _ = puts("attempt destroy a coroutine that has no free callback");
        return @bitCast(c_uint, MCO_INVALID_POINTER);
    }
    co.*.free_cb.?(@ptrCast(?*anyopaque, co), co.*.allocator_data);
    return @bitCast(c_uint, MCO_SUCCESS);
}
pub export fn mco_resume(arg_co: [*c]mco_coro) mco_result {
    var co = arg_co;
    if (!(co != null)) {
        _ = puts("attempt to resume an invalid coroutine");
        return @bitCast(c_uint, MCO_INVALID_COROUTINE);
    }
    if (co.*.state != @bitCast(c_uint, MCO_SUSPENDED)) {
        _ = puts("attempt to resume a coroutine that is not suspended");
        return @bitCast(c_uint, MCO_NOT_SUSPENDED);
    }
    co.*.state = @bitCast(c_uint, MCO_RUNNING);
    _mco_jumpin(co);
    return @bitCast(c_uint, MCO_SUCCESS);
}
pub export fn mco_yield(arg_co: [*c]mco_coro) mco_result {
    var co = arg_co;
    if (!(co != null)) {
        _ = puts("attempt to yield an invalid coroutine");
        return @bitCast(c_uint, MCO_INVALID_COROUTINE);
    }
    var dummy: usize = undefined;
    var stack_addr: usize = @intCast(usize, @ptrToInt(&dummy));
    var stack_min: usize = @intCast(usize, @ptrToInt(co.*.stack_base));
    var stack_max: usize = stack_min +% co.*.stack_size;
    if (((co.*.magic_number != @bitCast(c_ulong, @as(c_long, @as(c_int, 2117906857)))) or (stack_addr < stack_min)) or (stack_addr > stack_max)) {
        _ = puts("coroutine stack overflow, try increasing the stack size");
        return @bitCast(c_uint, MCO_STACK_OVERFLOW);
    }
    if (co.*.state != @bitCast(c_uint, MCO_RUNNING)) {
        _ = puts("attempt to yield a coroutine that is not running");
        return @bitCast(c_uint, MCO_NOT_RUNNING);
    }
    co.*.state = @bitCast(c_uint, MCO_SUSPENDED);
    _mco_jumpout(co);
    return @bitCast(c_uint, MCO_SUCCESS);
}
pub export fn mco_status(arg_co: [*c]mco_coro) mco_state {
    var co = arg_co;
    if (co != @ptrCast([*c]mco_coro, @alignCast(@import("std").meta.alignment([*c]mco_coro), @intToPtr(?*anyopaque, @as(c_int, 0))))) {
        return co.*.state;
    }
    return @bitCast(c_uint, MCO_DEAD);
}
pub export fn mco_get_user_data(arg_co: [*c]mco_coro) ?*anyopaque {
    var co = arg_co;
    if (co != @ptrCast([*c]mco_coro, @alignCast(@import("std").meta.alignment([*c]mco_coro), @intToPtr(?*anyopaque, @as(c_int, 0))))) {
        return co.*.user_data;
    }
...
pub export fn mco_push(arg_co: [*c]mco_coro, arg_src: ?*const anyopaque, arg_len: usize) mco_result {
    var co = arg_co;
    var src = arg_src;
    var len = arg_len;
    if (!(co != null)) {
        _ = puts("attempt to use an invalid coroutine");
        return @bitCast(c_uint, MCO_INVALID_COROUTINE);
    } else if (len > @bitCast(c_ulong, @as(c_long, @as(c_int, 0)))) {
        var bytes_stored: usize = co.*.bytes_stored +% len;
        if (bytes_stored > co.*.storage_size) {
            _ = puts("attempt to push too many bytes into coroutine storage");
            return @bitCast(c_uint, MCO_NOT_ENOUGH_SPACE);
        }
        if (!(src != null)) {
            _ = puts("attempt push a null pointer into coroutine storage");
            return @bitCast(c_uint, MCO_INVALID_POINTER);
        }
        _ = memcpy(@ptrCast(?*anyopaque, &co.*.storage[co.*.bytes_stored]), src, len);
        co.*.bytes_stored = bytes_stored;
    }
    return @bitCast(c_uint, MCO_SUCCESS);
}
pub export fn mco_pop(arg_co: [*c]mco_coro, arg_dest: ?*anyopaque, arg_len: usize) mco_result {
    var co = arg_co;
    var dest = arg_dest;
    var len = arg_len;
    if (!(co != null)) {
        ...
        if (len > co.*.bytes_stored) {
            _ = puts("attempt to pop too many bytes from coroutine storage");
            return @bitCast(c_uint, MCO_NOT_ENOUGH_SPACE);
        }
        var bytes_stored: usize = co.*.bytes_stored -% len;
        if (dest != null) {
            _ = memcpy(dest, @ptrCast(?*const anyopaque, &co.*.storage[bytes_stored]), len);
        }
        co.*.bytes_stored = bytes_stored;
        _ = memset(@ptrCast(?*anyopaque, &co.*.storage[bytes_stored]), @as(c_int, 0), len);
    }
    return @bitCast(c_uint, MCO_SUCCESS);
}
pub export fn mco_peek(arg_co: [*c]mco_coro, arg_dest: ?*anyopaque, arg_len: usize) mco_result {
    var co = arg_co;
    var dest = arg_dest;
    var len = arg_len;
    if (!(co != null)) {
        ...
        if (len > co.*.bytes_stored) {
            _ = puts("attempt to peek too many bytes from coroutine storage");
            return @bitCast(c_uint, MCO_NOT_ENOUGH_SPACE);
        }
        if (!(dest != null)) {
            _ = puts("attempt peek into a null pointer");
            return @bitCast(c_uint, MCO_INVALID_POINTER);
        }
        _ = memcpy(dest, @ptrCast(?*const anyopaque, &co.*.storage[co.*.bytes_stored -% len]), len);
    }
    return @bitCast(c_uint, MCO_SUCCESS);
}
pub export fn mco_get_bytes_stored(arg_co: [*c]mco_coro) usize {
    var co = arg_co;
    if (co == @ptrCast([*c]mco_coro, @alignCast(@import("std").meta.alignment([*c]mco_coro), @intToPtr(?*anyopaque, @as(c_int, 0))))) {
        return 0;
    }
...
pub export fn mco_get_storage_size(arg_co: [*c]mco_coro) usize {
    var co = arg_co;
    if (co == @ptrCast([*c]mco_coro, @alignCast(@import("std").meta.alignment([*c]mco_coro), @intToPtr(?*anyopaque, @as(c_int, 0))))) {
        return 0;
    }
...
pub export fn mco_running() [*c]mco_coro {
    var func: ?*const fn () callconv(.C) [*c]mco_coro = &_mco_running;
    return func.?();
}
pub export fn mco_result_description(arg_res: mco_result) [*c]const u8 {
    var res = arg_res;
    while (true) {
        switch (res) {
            @bitCast(c_uint, @as(c_int, 0)) => return "No error",
            @bitCast(c_uint, @as(c_int, 1)) => return "Generic error",
            @bitCast(c_uint, @as(c_int, 2)) => return "Invalid pointer",
            @bitCast(c_uint, @as(c_int, 3)) => return "Invalid coroutine",
            @bitCast(c_uint, @as(c_int, 4)) => return "Coroutine not suspended",
            @bitCast(c_uint, @as(c_int, 5)) => return "Coroutine not running",
            @bitCast(c_uint, @as(c_int, 6)) => return "Make context error",
            @bitCast(c_uint, @as(c_int, 7)) => return "Switch context error",
            @bitCast(c_uint, @as(c_int, 8)) => return "Not enough space",
            @bitCast(c_uint, @as(c_int, 9)) => return "Out of memory",
            @bitCast(c_uint, @as(c_int, 10)) => return "Invalid arguments",
            @bitCast(c_uint, @as(c_int, 11)) => return "Invalid operation",
            @bitCast(c_uint, @as(c_int, 12)) => return "Stack overflow",
            else => {},
        }
        break;
    }
...
pub fn mco_malloc(arg_size: usize, arg_allocator_data: ?*anyopaque) callconv(.C) ?*anyopaque {
    var size = arg_size;
    var allocator_data = arg_allocator_data;
    _ = @TypeOf(allocator_data);
    return malloc(size);
}
pub fn mco_free(arg_ptr: ?*anyopaque, arg_allocator_data: ?*anyopaque) callconv(.C) void {
    var ptr = arg_ptr;
    var allocator_data = arg_allocator_data;
    _ = @TypeOf(allocator_data);
    free(ptr);
...
pub inline fn _mco_align_forward(arg_addr: usize, arg_align: usize) usize {
    var addr = arg_addr;
    var @"align" = arg_align;
    return (addr +% (@"align" -% @bitCast(c_ulong, @as(c_long, @as(c_int, 1))))) & ~(@"align" -% @bitCast(c_ulong, @as(c_long, @as(c_int, 1))));
}
pub threadlocal var mco_current_co: [*c]mco_coro = null;
pub inline fn _mco_prepare_jumpin(arg_co: [*c]mco_coro) void {
    var co = arg_co;
    var prev_co: [*c]mco_coro = mco_running();
    _ = blk: {
        _ = @sizeOf(c_int);
        break :blk blk_1: {
            break :blk_1 if (co.*.prev_co == @ptrCast([*c]mco_coro, @alignCast(@import("std").meta.alignment([*c]mco_coro), @intToPtr(?*anyopaque, @as(c_int, 0))))) {} else {
                __assert_fail("co->prev_co == ((void*)0)", "./minicoro.h", @bitCast(c_uint, @as(c_int, 481)), "void _mco_prepare_jumpin(mco_coro *)");
            };
        };
    };
    co.*.prev_co = prev_co;
    if (prev_co != null) {
        _ = blk: {
            _ = @sizeOf(c_int);
            break :blk blk_1: {
                break :blk_1 if (prev_co.*.state == @bitCast(c_uint, MCO_RUNNING)) {} else {
                    __assert_fail("prev_co->state == MCO_RUNNING", "./minicoro.h", @bitCast(c_uint, @as(c_int, 484)), "void _mco_prepare_jumpin(mco_coro *)");
                };
            };
        };
        prev_co.*.state = @bitCast(c_uint, MCO_NORMAL);
    }
    mco_current_co = co;
}
pub inline fn _mco_prepare_jumpout(arg_co: [*c]mco_coro) void {
    var co = arg_co;
    var prev_co: [*c]mco_coro = co.*.prev_co;
    co.*.prev_co = null;
    if (prev_co != null) {
        prev_co.*.state = @bitCast(c_uint, MCO_RUNNING);
    }
    mco_current_co = prev_co;
}
pub const struct__mco_ctxbuf = extern struct {
    rip: ?*anyopaque,
    rsp: ?*anyopaque,
    rbp: ?*anyopaque,
    rbx: ?*anyopaque,
...
pub const _mco_ctxbuf = struct__mco_ctxbuf;
pub const struct__mco_context = extern struct {
    ctx: _mco_ctxbuf,
    back_ctx: _mco_ctxbuf,
};
pub const _mco_context = struct__mco_context;
pub fn _mco_jumpin(arg_co: [*c]mco_coro) callconv(.C) void {
    var co = arg_co;
    var context: [*c]_mco_context = @ptrCast([*c]_mco_context, @alignCast(@import("std").meta.alignment([*c]_mco_context), co.*.context));
    _mco_prepare_jumpin(co);
    _ = _mco_switch(&context.*.back_ctx, &context.*.ctx);
}
pub fn _mco_jumpout(arg_co: [*c]mco_coro) callconv(.C) void {
    var co = arg_co;
    var context: [*c]_mco_context = @ptrCast([*c]_mco_context, @alignCast(@import("std").meta.alignment([*c]_mco_context), co.*.context));
    _mco_prepare_jumpout(co);
    _ = _mco_switch(&context.*.ctx, &context.*.back_ctx);
}
pub fn _mco_main(arg_co: [*c]mco_coro) callconv(.C) void {
    var co = arg_co;
    co.*.func.?(co);
    co.*.state = @bitCast(c_uint, MCO_DEAD);
    _mco_jumpout(co);
}
pub extern fn _mco_wrap_main() void;
pub extern fn _mco_switch(from: [*c]_mco_ctxbuf, to: [*c]_mco_ctxbuf) c_int;
comptime {
...
pub fn _mco_makectx(arg_co: [*c]mco_coro, arg_ctx: [*c]_mco_ctxbuf, arg_stack_base: ?*anyopaque, arg_stack_size: usize) callconv(.C) mco_result {
    var co = arg_co;
    var ctx = arg_ctx;
    var stack_base = arg_stack_base;
    var stack_size = arg_stack_size;
    stack_size = stack_size -% @bitCast(c_ulong, @as(c_long, @as(c_int, 128)));
    ...
    ctx.*.rip = @ptrCast(?*anyopaque, &_mco_wrap_main);
    ctx.*.rsp = @ptrCast(?*anyopaque, stack_high_ptr);
    ctx.*.r12 = @ptrCast(?*anyopaque, &_mco_main);
    ctx.*.r13 = @ptrCast(?*anyopaque, co);
    return @bitCast(c_uint, MCO_SUCCESS);
}
pub fn _mco_create_context(arg_co: [*c]mco_coro, arg_desc: [*c]mco_desc) callconv(.C) mco_result {
    var co = arg_co;
    var desc = arg_desc;
    var co_addr: usize = @intCast(usize, @ptrToInt(co));
    var context_addr: usize = _mco_align_forward(co_addr +% @sizeOf(mco_coro), @bitCast(usize, @as(c_long, @as(c_int, 16))));
    var storage_addr: usize = _mco_align_forward(context_addr +% @sizeOf(_mco_context), @bitCast(usize, @as(c_long, @as(c_int, 16))));
    var stack_addr: usize = _mco_align_forward(storage_addr +% desc.*.storage_size, @bitCast(usize, @as(c_long, @as(c_int, 16))));
    var context: [*c]_mco_context = @intToPtr([*c]_mco_context, context_addr);
    _ = memset(@ptrCast(?*anyopaque, context), @as(c_int, 0), @sizeOf(_mco_context));
    var storage: [*c]u8 = @intToPtr([*c]u8, storage_addr);
    _ = memset(@ptrCast(?*anyopaque, storage), @as(c_int, 0), desc.*.storage_size);
    var stack_base: ?*anyopaque = @intToPtr(?*anyopaque, stack_addr);
    var stack_size: usize = desc.*.stack_size;
    _ = memset(stack_base, @as(c_int, 0), stack_size);
    var res: mco_result = _mco_makectx(co, &context.*.ctx, stack_base, stack_size);
    if (res != @bitCast(c_uint, MCO_SUCCESS)) {
        return res;
    }
    co.*.context = @ptrCast(?*anyopaque, context);
...
pub fn _mco_destroy_context(arg_co: [*c]mco_coro) callconv(.C) void {
    var co = arg_co;
    _ = @TypeOf(co);
}
pub inline fn _mco_init_desc_sizes(arg_desc: [*c]mco_desc, arg_stack_size: usize) void {
    var desc = arg_desc;
    var stack_size = arg_stack_size;
    desc.*.coro_size = (((_mco_align_forward(@sizeOf(mco_coro), @bitCast(usize, @as(c_long, @as(c_int, 16)))) +% _mco_align_forward(@sizeOf(_mco_context), @bitCast(usize, @as(c_long, @as(c_int, 16))))) +% _mco_align_forward(desc.*.storage_size, @bitCast(usize, @as(c_long, @as(c_int, 16))))) +% stack_size) +% @bitCast(c_ulong, @as(c_long, @as(c_int, 16)));
    desc.*.stack_size = stack_size;
}
pub fn _mco_validate_desc(arg_desc: [*c]mco_desc) callconv(.C) mco_result {
    var desc = arg_desc;
    if (!(desc != null)) {
        _ = puts("coroutine description is NULL");
        return @bitCast(c_uint, MCO_INVALID_ARGUMENTS);
    }
    if (!(desc.*.func != null)) {
        _ = puts("coroutine function in invalid");
        return @bitCast(c_uint, MCO_INVALID_ARGUMENTS);
    }
    if (desc.*.stack_size < @bitCast(c_ulong, @as(c_long, @as(c_int, 32768)))) {
        _ = puts("coroutine stack size is too small");
        return @bitCast(c_uint, MCO_INVALID_ARGUMENTS);
    }
    if (desc.*.coro_size < @sizeOf(mco_coro)) {
        _ = puts("coroutine size is invalid");
        return @bitCast(c_uint, MCO_INVALID_ARGUMENTS);
    }
    return @bitCast(c_uint, MCO_SUCCESS);
}
pub fn _mco_running() callconv(.C) [*c]mco_coro {
...
pub export fn coro_entry(arg_co: [*c]mco_coro) void {
    ...
    _ = puts(@ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), &buffer)));
    ret = 1;
    _ = blk: {
        _ = @sizeOf(c_int);
        break :blk blk_1: {
...
pub const MCO_API = @compileError("unable to translate C expr: unexpected token 'extern'"); // ./minicoro.h:207:9
pub const offsetof = @compileError("unable to translate macro: undefined identifier `__builtin_offsetof`"); // /home/user/.local/share/zig/0.11.0-dev.1898+36d47dd19/files/lib/include/stddef.h:104:9
...
pub const va_start = @compileError("unable to translate macro: undefined identifier `__builtin_va_start`"); // /home/user/.local/share/zig/0.11.0-dev.1898+36d47dd19/files/lib/include/stdarg.h:17:9
pub const va_end = @compileError("unable to translate macro: undefined identifier `__builtin_va_end`"); // /home/user/.local/share/zig/0.11.0-dev.1898+36d47dd19/files/lib/include/stdarg.h:18:9
pub const va_arg = @compileError("unable to translate macro: undefined identifier `__builtin_va_arg`"); // /home/user/.local/share/zig/0.11.0-dev.1898+36d47dd19/files/lib/include/stdarg.h:19:9
pub const __va_copy = @compileError("unable to translate macro: undefined identifier `__builtin_va_copy`"); // /home/user/.local/share/zig/0.11.0-dev.1898+36d47dd19/files/lib/include/stdarg.h:24:9
pub const va_copy = @compileError("unable to translate macro: undefined identifier `__builtin_va_copy`"); // /home/user/.local/share/zig/0.11.0-dev.1898+36d47dd19/files/lib/include/stdarg.h:27:9
...
pub const MCO_THREAD_LOCAL = @compileError("unable to translate C expr: unexpected token '_Thread_local'"); // ./minicoro.h:389:15
pub const MCO_FORCE_INLINE = @compileError("unable to translate macro: undefined identifier `__attribute__`"); // ./minicoro.h:408:15
pub const MCO_NO_INLINE = @compileError("unable to translate macro: undefined identifier `__attribute__`"); // ./minicoro.h:419:13
...
pub const MCO_ZERO_MEMORY = "";
pub const MINICORO_IMPL = "";
pub const MINICORO_H = "";
pub const MCO_DEFAULT_STORAGE_SIZE = @as(c_int, 1024);
...
pub const MCO_MIN_STACK_SIZE = @import("std").zig.c_translation.promoteIntLiteral(c_int, 32768, .decimal);
pub const MCO_DEFAULT_STACK_SIZE = @import("std").zig.c_translation.promoteIntLiteral(c_int, 57344, .decimal);
pub const MCO_MAGIC_NUMBER = @import("std").zig.c_translation.promoteIntLiteral(c_int, 0x7E3CB1A9, .hexadecimal);
pub const MCO_USE_ASM = "";
pub const _MCO_UNUSED = @import("std").zig.c_translation.Macros.DISCARD;
pub const MCO_DEBUG = "";
...
pub inline fn MCO_LOG(s: anytype) @TypeOf(puts(s)) {
    return puts(s);
}
pub const _ASSERT_H = @as(c_int, 1);
pub inline fn MCO_ASSERT(c: anytype) @TypeOf(assert(c)) {
    return assert(c);
}
...
pub const MCO_MALLOC = malloc;
pub const MCO_FREE = free;
